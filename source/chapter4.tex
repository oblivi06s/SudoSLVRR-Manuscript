...
\subsection*{Success Rate (SR)}
        ...
        \end{equation}
        where $N_{total}$ is 100 unique general puzzles. A higher success rate indicates improved exploration as multi-colony collaboration and inter-thread communication preserves diversity, effectively reducing stagnation.     
...
\subsection{Experiment 2: Single-Threaded Multi-Colony ACO vs Multithreaded Multi-Colony ACO} 
	This experiment compares the performance of between single-threaded and multithreaded solvers. This highlights the impact of parallelization on the solver’s performance when solving Sudoku puzzles. It is important to note that parallelization in this experiment involves collaboration with other threads. 
...
\subsection{Experiment 2: Single-Threaded Multi-Colony ACO vs Multithreaded Multi-Colony ACO} 
        This experiment shows the difference of the performance of single-threaded multi-colony ACO and multithreaded multi-colony ACO. Furthermore, conducting this experiment shows the advantage of using concurrent exploration when solving combinatorial search space. 
        Table~\ref{tab:single_vs_multi_comparison} shows the results of running the single-threaded multi-colony ACO (CP-DCM-ACO) and the multithreaded multi-colony ACO (MT-CP-DCM-ACO). The solvers processed the same set of instances consisting of 100 puzzles each grid size ($9\times9$, $16\times16$, and $25\times25$) 5 times. We can see from the table that multithreading does not improves the speed of the solver but also offers greater scalability. It is clear that by leveraging parallel processing, the solver can guarantee finding a solution even in extreme solution space. 

\begin{table}[H]
    \centering
    \caption{Performance comparison in terms of success rate (\%), average runtime (seconds), and average number of iterations across puzzle sizes. }
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}
        ...
        \end{axis}
    \end{tikzpicture}
    \caption{Comparison of computational efficiency (runtime in seconds and iteration count) between CP-DCM-ACO and MT-CP-DCM-ACO across different puzzle sizes. }
    \label{fig:performance_comparison}
\end{figure}

        Furthermore, Figure~\ref{fig:performance_comparison} shows that the computational costs of the MT-CP-DCM-ACO are still less than that of the CP-DCM-ACO even though additional overhead due to thread communication is introduced in the multithreaded solver. This result shows that multithreaded solver explores more regions of the solutions space thus speeding up the search for a solution. 
...
\begin{table}[H]
    \centering
    \caption{Impact of thread communication on the MT-CP-DCM-ACO performance in terms of success rate (\%), average runtime (seconds), and average number of iterations across puzzle sizes.}
