...
Figure~\ref{general} shows the general overview of the methodology from data gathering to model deployment. Inputting the Sudoku puzzles retrieved from publicly available dataset with different sizes and number of clues into the constraint propagation defines the initial step of the process. Constraint propagation eliminates infeasible assignments in the puzzle, reducing the search space and improving the efficiency of the optimization phase by preventing the ants from exploring regions of the search space that contain invalid solutions. After that, the proposed model, collaborative multithreaded multi-colony ant optimization, receives the refined puzzle. The collaborative mechanism between threads is periodically activated to enhance the search process for a valid solution. The execution is terminated when any thread successfully generates a valid Sudoku solution.  The proposed model is then evaluated based on performance metrics namely solution success rate, average completion time, standard deviation, and average number of cycles. 

\section{Dataset}
The study used publicly available $9\times9$ Sudoku puzzles retrieved from The New Sudoku Players’ Forum (http://forum.enjoysudoku.com/the-hardest-sudokus-new-thread-t6539.html) and $16\times16$ Sudoku datasets from an open-source Github repository (https://github.com/goomii17/16X16-Sudoku-Dataset) to ensure reproducibility. For large 25 x 25 grids, Sudoku instances of this size were obtained from the work of Lloyd and Amos~\cite{lloyd2020}. The baseline solver (CP-ACS) is then used to solve these puzzles and selected the 100 most difficult instances for the final benchmark instances. The selection was based on the computational effort and solving time of the solver. The final benchmark dataset is composed of 300 Sudoku instances which is partitioned into 100 selected hard puzzles for each puzzle size, providing diverse puzzle complexity to evaluate the solver’s scalability and adaptability. 
            
All these Sudoku puzzles are stored in a text file that includes the position of the clues. The ratio between the number of clues and the puzzle size defines the complexity
...
The constraint propagation (CP) is the one that acts like a preprocessing step of the proposed  multithreaded multi-colony ant optimization model with ring and random communication topologies. It narrows down the search space by removing infeasible cell-digit assignments, thereby creating a refined starting search space and minimizing computational effort for the optimization phase. In the context of Sudoku, rules are established and should be followed. Each cell must contain only one digit while satisfying these three simultaneous constraints:
        \begin{enumerate}
...
The proposed framework starts with the initialization phase where necessary variables should be set before any construction of solution begins. These variables are the necessary components as it affects the overall performance of the solver. 
    ...
    \begin{enumerate}
        \item \textbf{Number of Threads} - This includes the threads setup and its number. Threads are the ones that hold the main solver in this framework.
...
\subsection{CP-DCM-ACO}
  ...
  A local copy of the puzzle from the CP is given to each ant. Then, these ants start from a random cell and traverse the grid from left to right to ensure all decision variables are evaluated while satisfying the Sudoku constraints as shown in the Figure~\ref{Ants start}. 
    ...
    When the initial pass to the CP is done, the ants start their tour. The ants visit cell $i$ and check if the cell has already a fixed value assigned to it. If there is none, the ants look at the ``value set'' $v_i$, which contains all the valid candidate values for that cell. To choose a value $s$ from this set, ant depends on a random variable $q$ and compare this to $q_0$ which is a tunable parameter and whose value must be between $[0, 1]$. This random variable is critical for preventing premature convergence as it balances the solver’s exploitation of promising assignments (greedy selection) and exploration of undiscovered regions in the search space (probabilistic selection). The value selection s is defined as follows:
...
When $q \ge q_0$, the ants perform the roulette wheel selection instead of greedy selection (See Figure~\ref{Ant Greedy}). As opposed to the greedy selection, ants do not assign immediately the value that has the highest pheromone value for a certain cell. Instead, it calculates the probability $p_{i}^{k}$ of choosing the value $k$ and assigning it to the cell $i$ as shown in Figure~\ref{Ant Roulette}. This probability favors values with higher pheromone concentration while still allowing exploration by maintaining fractional probabilities for alternative assignments. Unlike the greedy selection, the value $s$ with low pheromone value still has a chance to be assigned to cell $i$.  The probability distribution is given by:
...
\begin{center}
    \begin{minipage}{\linewidth}
        \centering
        \includegraphics[width=0.9\linewidth]{../figures/Greedy Selection.png} 
        \captionof{figure}{Ants perform greedy selection} 
        \label{Ant Greedy} 
        
        \vspace{1.0cm} % Slightly smaller gap
        
        \includegraphics[width=0.9\linewidth]{../figures/Probabilities.png} 
        \captionof{figure}{Ants perform roulette wheel selection when $q \ge q_0$} 
        \label{Ant Roulette} 
    \end{minipage}
\end{center}
...
% --- CP Rule 1 ---
\begin{center}
    \includegraphics[width=0.7\linewidth]{../figures/CP in ACS Rule 1 a.png} 
    \captionof{figure}{Before applying CP Rule 1} 
    \label{fig:cpacs_step_1a} 
    
    \vspace{0.5cm} % Gap between images
    
    \includegraphics[width=0.7\linewidth]{../figures/CP in ACS Rule 1 b.png} 
    \captionof{figure}{After applying CP Rule 1} 
    \label{fig:cpacs_step_1b} 
\end{center}

\vspace{0.5cm} % Gap between Rule 1 and Rule 2

% --- CP Rule 2 ---
\begin{center}
    \includegraphics[width=0.7\linewidth]{../figures/CP in ACS Rule 2 a.png} 
    \captionof{figure}{Before applying CP Rule 2} 
    \label{fig:cpacs_step_2a} 
    
    \vspace{0.5cm} % Gap between images
    
    \includegraphics[width=0.7\linewidth]{../figures/CP in ACS Rule 2 b.png} 
    \captionof{figure}{After applying CP Rule 2} 
